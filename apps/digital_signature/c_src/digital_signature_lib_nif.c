...
static ERL_NIF_TERM CheckCertOnline(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
   ...
   {
     return CreateErrorTupple(env, "content data argument is of incorrect type: must be Elixir string (binary)");
   }

   ErlNifBinary ocspCertData;
   if (!enif_inspect_binary(env, argv[1], &ocspCertData))
   {
     return CreateErrorTupple(env, "ocsp certificate data argument is of incorrect type: must be Elixir string (binary)");
   }

   ErlNifBinary url;
   if (!enif_inspect_binary(env, argv[2], &url))
   {
     return CreateErrorTupple(env, "url argument is of incorrect type: must be Elixir string (binary)");
   }

   UAC_BLOB ocspDataRequest = {contentData.data, contentData.size};
   UAC_BLOB ocspCert = {ocspCertData.data, ocspCertData.size};
...
static ErlNifFunc nif_funcs[] = {
    ...
    {"checkCertOnline",  3, CheckCertOnline,  ERL_NIF_DIRTY_JOB_CPU_BOUND},
